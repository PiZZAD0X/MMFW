...
class EGVAR(EndConditions,Settings_Category1) {
    ...
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_1_Menu););
};

class EGVAR(EndConditions,Settings_Category2) {
    text = "Configure Condition Category 2";
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_2_Menu););
};

class EGVAR(EndConditions,Settings_Category3) {
    text = "Configure Condition Category 3";
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_3_Menu););
};

class EGVAR(EndConditions,Settings_Category4) {
    text = "Configure Condition Category 4";
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_4_Menu););
};

class EGVAR(EndConditions,Settings_Category5) {
    text = "Configure Condition Category 5";
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_5_Menu););
};

class EGVAR(EndConditions,Settings_Category6) {
    text = "Configure Condition Category 6";
    action = QUOTE(edit3DENMissionAttributes QQEGVAR(EndConditions,Category_6_Menu););
};
