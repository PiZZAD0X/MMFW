        ...
        if ((_conditionsCountCategory6) > 0  || ((GETMVAR(ExtractionEnabled_1,false)) && {(GETMVAR(ExtractionForced_1,false))})) then {
            ...
            private _endConditionsCategory6PFHhandle = [{
                ...
                if (GETMVAR(ExtractionEnabled_1,false)) then {
                    private _team = ([GVAR(TeamName_Blufor),GVAR(TeamName_Opfor),GVAR(TeamName_Indfor),GVAR(TeamName_Civ)] select GVAR(ExtractionTeam_1));
                    if (GVAR(ExtractionMarker_1) isEqualto "") exitwith {["","No marker entered for extract zone for Category 1!"] call FUNC(DebugMessageDetailed);};
                    if ((getMarkerColor GVAR(ExtractionMarker_1)) isEqualto "") exitwith {["","Invalid extract marker for Category 1!"] call FUNC(DebugMessageDetailed);};
                        ...
                        TRACE_1("Extraction Check Category 1",_ExtractionCheck);
                } else {
                    _ExtractionCheck = true;
                };
