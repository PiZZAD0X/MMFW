if (GETMVAR(Enabled_3,false)) then {
    if ((isNil QGVAR(Message_3)) || {!(GETMVAR(Message_3,"") isEqualType "STRING")}) then {
        ...
        private _conditionsCountCategory_3 = 0;
        if (GETMVAR(CasualtyCount_Enabled_BLUFOR_3,false)) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (GETMVAR(CasualtyCount_Enabled_OPFOR_3,false)) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (GETMVAR(CasualtyCount_Enabled_Indfor_3,false)) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (GETMVAR(CasualtyCount_Enabled_Civ_3,false)) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};

        if (!(GETMVAR(EntitiesAlive_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (!(GETMVAR(EntitiesDead_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (!(GETMVAR(EntitiesDamaged_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (!(GETMVAR(HostageRescued_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (!(GETMVAR(CustomVariables_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};
        if (!(GETMVAR(CaptureZoneCaptured_Array_3,[]) isEqualto [])) then {_conditionsCountCategory_3 = _conditionsCountCategory_3 + 3;};

        if ((_conditionsCountCategory_3) > 0  || ((GETMVAR(ExtractionEnabled_3,false)) && {(GETMVAR(ExtractionForced_3,false))})) then {
            LOG_3("Starting Category 3 Condition Count:%1",_conditionsCountCategory_3);
            GVAR(Category_3_PFH) = [{
                ...
                if (!(_aliveUnitArray isEqualto [])) then {
                    LOG_3("Alive Array 3:%1",_aliveUnitArray);
                    private _alive = true;
                    {
                ...
                if (!(_deadUnitArray isEqualto [])) then {
                    LOG_3("Dead Array 3:%1",_deadUnitArray);
                    private _dead = true;
                    {
                        private _unit = missionNamespace getVariable [_x,objNull];
                ...
                if (!(_damagedUnitArray isEqualto [])) then {
                    LOG_3("Damaged Array 3:%1",_damagedUnitArray);
                ...
                if (!(_hostageRescuedArray isEqualto [])) then {
                    LOG_3("Rescued Array 3:%1",_hostageRescuedArray);
                ...
                if (!(_customVariablesArray isEqualto [])) then {
                    LOG_3("Var Array 3:%1",_customVariablesArray);
                ...
                if (!(_captureZones_Array_3 isEqualto [])) then {
                    private _captureZones_TeamSetting_3 = GETMVAR(CaptureZoneCaptured_Team_3,1);
                    {
                        ...
                        if !((call compile _x) in (EGETMVAR(CaptureZone,ListArray,[]))) then {
                            LOG_3("CaptureZone %1 does not exist!",_x);
                ...
                LOG_3("Category 3 _ConditionCheckList:%1",_ConditionCheckList);
                ...
                if (GETMVAR(ExtractionEnabled_3,false)) then {
                    ...
                    if ((getMarkerColor GVAR(ExtractionMarker_3)) isEqualto "") exitwith {["","Invalid extract marker for Category 3!"] call FUNC(DebugMessageDetailed);};
                        ...
                        TRACE_3("Extraction Check Category 3",_ExtractionCheck);
                } else {
                    _ExtractionCheck = true;
                };
                ...
                if (_ExtractionCheck) then {
                    ...
                    {
                        LOG_3("Category 3 Ending due to forced Extraction!");
                        [GVAR(Message_3)] call FUNC(EndMission);
                        [_idPFH] call CBA_fnc_removePerFrameHandler;
                    } else {
                        if (GVAR(Mode_3) isEqualto 3) then {
                            {
                                _x params ["_name","_value"];
                                if (_value) exitwith {
                                    LOG_3("Category 3 Ending due to :%1",_value);
                                    [GVAR(Message_3)] call FUNC(EndMission);
                                    [_idPFH] call CBA_fnc_removePerFrameHandler;
                                };
