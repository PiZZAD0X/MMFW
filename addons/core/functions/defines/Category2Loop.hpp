if (GETMVAR(Enabled_2,false)) then {
    if ((isNil QGVAR(Message_2)) || {!(GETMVAR(Message_2,"") isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_Enabled_BLUFOR_2,false)) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (GETMVAR(CasualtyCount_Enabled_OPFOR_2,false)) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (GETMVAR(CasualtyCount_Enabled_Indfor_2,false)) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (GETMVAR(CasualtyCount_Enabled_Civ_2,false)) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};

        if (!(GETMVAR(EntitiesAlive_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (!(GETMVAR(EntitiesDead_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (!(GETMVAR(EntitiesDamaged_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (!(GETMVAR(HostageRescued_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (!(GETMVAR(CustomVariables_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};
        if (!(GETMVAR(CaptureZoneCaptured_Array_2,[]) isEqualto [])) then {_conditionsCountCategory6 = _conditionsCountCategory6 + 1;};

        if ((_conditionsCountCategory6) > 0  || ((GETMVAR(ExtractionEnabled_2,false)) && {(GETMVAR(ExtractionForced_2,false))})) then {
            LOG_1("Starting Category 2 Condition Count:%1",_conditionsCountCategory6);
            private _endConditionsCategory6PFHhandle = [{
                ...
                if (GETMVAR(ExtractionEnabled_2,false)) then {
                    ...
                    if ((getMarkerColor GVAR(ExtractionMarker_2)) isEqualto "") exitwith {["","Invalid extract marker for Category 2!"] call FUNC(DebugMessageDetailed);};
                        ...
                        TRACE_1("Extraction Check Category 2",_ExtractionCheck);
                } else {
                    _ExtractionCheck = true;
                };
